...
export default class PetProfile extends Component {
    ...
    removePhoto(removedPhoto){
        ...
        let newPhotos = oldPhotos.filter(photo =>
            photo !== removedPhoto
                ...
                <div className="card">
                    <div className="card-header">
                        ...
                        <ul className="list-group list-group-flush">
                            <li className="list-group-item">Species: {this.state.pet.species}</li>
                            <li className="list-group-item">Breed: {this.state.pet.breeds.primary}</li>
                            <li className="list-group-item">Gender: {this.state.pet.gender}</li>
                            <li className="list-group-item">Age: {this.state.pet.age}</li>
                            <li className="list-group-item">From: {this.state.pet.contact.address.location.city}, {this.state.pet.contact.address.location.state}</li>
                        </ul>
                </div>
                 ...
                 <div className="card">
                     ...
                     <ul className="list-group list-group-flush">
                         <li className="list-group-item">
                             {!this.state.addPhoto &&
                              <span className="nav-link btn btn-outline-primary"
                                 onClick={this.addNewPhoto}>Add new Photo</span>
                             }
                             {this.state.addPhoto &&
                              <span>
                              <input placeholder="site link" className="nav-link" onChange={this.onChangePhoto}/>
                              <button className="nav-link btn btn-outline-primary" onClick={this.sendPetToServer}>Submit</button>
                                  <button className="nav-link btn btn-outline-danger" onClick={this.cancelAddNewPhoto}>Never Mind</button>
                              </span>
                             }

                         </li>
                         <li className="list-group-item">
                             <Link to={`/blog/${this.state.petId}`} className="nav-link btn btn-outline-success">Create BlogPost</Link>
                         </li>
                         <li className="list-group-item">
                             {!this.state.remove &&
                              <span  className="nav-link btn btn-outline-danger" onClick={this.removePet}>Remove {this.state.pet.name}</span>
                             }
                             {this.state.remove &&
                              <ul className="list-group list-group-flush">
                                  <li className="list-group-item">
                                      <span  className="nav-link btn btn-outline-primary" onClick={this.neverMind}>Never Mind</span>
                                  </li>
                                  <li className="list-group-item">
                                      <span className="nav-link btn btn-outline-danger" onClick={this.removePet}>Confirm Removal</span>
                                  </li>
                              </ul>
                             }
                         </li>
